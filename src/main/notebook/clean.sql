...
-- 正在导出表  sinitial.sl_post 的数据：~8 rows (大约)
...
INSERT INTO `sl_post` (`post_id`, `post_title`, `post_status`, `post_author`, `post_content`, `post_mime_type`, `post_date`, `post_type`) VALUES
	...
	(4, '快排の日常', 'publish', 2, '想象一个场景：有8本书，高度各不相同。咱想把这些书按从高到低的顺序摆到书架上，请务必帮咱想一个可靠的办法！\r\n\r\n> 依次比较相邻书本的高度（从左至右依次名为1号~8号，若2号比1号高，则交换两者位置；若1号比2号高，则不动。然后2号继续跟3号比较），直到最右。此时最右是最低的书，拿到一边，对剩下的书循环上述过程即可完成排序。\r\n\r\n此乃冒泡排序法\r\n\r\n> 随便选一本书，比这本书高的放左边，低的放右边；完成后以刚才选中的书为界分为2个部分；对每部分重复上述操作后划分为4部分；以此类推，直到每部分只有一本书，排序达成。\r\n\r\n此乃快速排序法，采用了分治的思想，分治法（Divide and conquer）用来将一个序列（list）分为两个子序列（sub-lists）。每次操作的对象比前一次少一半还多，效率优于冒泡。\r\n\r\n快速排序最早由东尼·霍尔在1962提出。在平均状况下，排序n个项目要`Ο(N*logN)`次比较。在最坏状况下则需要`O`次比较，但这种状况并不常见。\r\n\r\n具体步骤为：\r\n\r\n1. 从数列中挑出一个元素，称为"基准"（pivot），\r\n2. 重新排序数列，所有比基准值小的元素摆放在基准前面，所有比基准值大的元素摆在基准后面（相同的数可以到任一边）。在这个分区结束之后，该基准就处于数列的中间位置。这个称为分区（partition）操作。\r\n3. 递归地（recursively）把小于基准值元素的子数列和大于基准值元素的子数列排序。\r\n\r\n递归到最底部时，数列的大小是零或一，也就是已经排序好了。这个算法一定会结束，因为在每次的迭代（iteration）中，它至少会把一个元素摆到它最后的位置去。\r\n\r\n那么就决定是你了！回到现实，咱这里有个数组需要排序，那么如何将此方法用Java实现呢？\r\n\r\n```java\r\n// 快速排序\r\npublic int[] quickSort(int[] numbers, int low, int high) {\r\n    // 钦点中间为基准数\r\n    int pivot = numbers[low + (high - low) / 2];\r\n    // 默认为0\r\n    int i = low;\r\n    // 默认为数组长度-1\r\n    int j = high;\r\n    while (i <= j) {\r\n        while (numbers[i] < pivot) {\r\n            i++;\r\n        }\r\n        while (numbers[j] > pivot) {\r\n            j--;\r\n        }\r\n        if (i <= j) {\r\n            int temp = numbers[i];\r\n            numbers[i] = numbers[j];\r\n            numbers[j] = temp;\r\n            i++;\r\n            j--;\r\n        }\r\n    }\r\n    // 划分小区间，递归\r\n    if (i < high) {\r\n        quickSort(numbers, i, high);\r\n    }\r\n    if (low < j) {\r\n        quickSort(numbers, low, j);\r\n    }\r\n    return numbers;\r\n}\r\n```\r\n以上是最简单的写法，通过递归实现。\r\n\r\n未完待续~', 'article', '2017-09-24 17:58:58', '想象一个场景：有8本书，高度各不相同。咱想把这些书按从高到低的顺序摆到书架上，请务必帮咱想一个可靠的办法！\r\n\r\n> 依次比较相邻书本的高度（从左至右依次名为1号~8号，若2号比1号高，则交换两者位置；若1号比2号高，则不动。然后2号继续跟3号比较），直到最右。此时最右是最低的书，拿到一边，对剩下的书循环上述过程即可完成排序。\r\n\r\n此乃冒泡排序法\r\n\r\n> 随便选一本书，比这本书高的放左边，低的放右边；完成后以刚才选中的书为界分为2个部分；对每部分重复上述操作后划分为4部分；以此类推，直到每部分只有一本书，排序达成。\r\n\r\n此乃快速排序法，采用了分治的思想，分治法（Divide and conquer）用来将一个序列（list）分为两个子序列（sub-lists）。每次操作的对象比前一次少一半还多，效率优于冒泡。');
