<div class="white-wrapper"><img class="circle-profile"
                                src="${root}/resources/img/head_img.jpg">
    <p class="site-description">咸鱼王...先这样。</p>
    <div class="grey-rule"></div>
    <ul>
        <li><a href="/" target="_self">主页</a></li>

        <li><a href="${root}/user/login/page" target="_self">管理</a></li>

        <li><a href="###">关于我</a></li>
    </ul>
    <div class="grey-rule"></div>
    <div class="social-link-group">
        <a class="social-icon-link"
           href="https://github.com/nekuata" target="_blank">
            <i class="fa fa-github fa-2x"></i>
        </a>
        <a class="social-icon-link" href="#">
            <i class="fa fa-envelope fa-2x"></i>
        </a>
        <a class="social-icon-link" href="#">
            <i class="fa fa-telegram fa-2x"></i>
        </a>
        <a class="social-icon-link" href="#">
            <i class="fa fa-qq fa-2x"></i>
        </a>
    </div>
    <p class="copyright-text">site design © 2017
        <a target="_self" href="https://meowwoo.com"
           class="text-link">nekuata</a>
    </p>
</div>